...
class StochasticPolicyGradientAgent():
    ...
    """
    def __init__(self, env, learning_rate = 0.0001, discount_rate = 0.99):
        ...
        mu_hidden = tf.layers.dense(state, h1, 
        ...
        sig_hidden = tf.layers.dense(state, h1, 
                                     activation = tf.sigmoid, 
                                     name = 'sigma_dense', 
                                     kernel_initializer=init)
        ...
        self._loss = -tf.log(1e-5 + self._normal_dist.prob(self._taken_actions)) * self._discounted_rewards
